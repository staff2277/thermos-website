...
export default function ReviewsSection() {
  ...
  return (
    <section ref={containerRef} className="relative w-full min-h-screen bg-black overflow-hidden flex flex-col justify-center py-24 md:py-32">
      <div className="max-w-[95%] mx-auto px-6 md:px-12 lg:px-24 mb-20 flex flex-col gap-6">
        <div className="flex items-center gap-3">
          ...
          <span className="text-accent/60 font-bold tracking-[0.3em] uppercase text-xs">
            Global Recognition
          </span>
        </div>
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <h2 className="text-6xl md:text-7xl font-black tracking-tighter leading-tight">
            WORLD CLASS <br />
            <span className="text-white/20">TESTIMONIALS.</span>
          </h2>
          <Link 
            href="/reviews"
      ...
      <div ref={scrollRef} className="flex gap-8 px-6 md:px-12 lg:px-24">
        {reviews.slice(0, 6).map((review) => (
          ...
          >
            ...
            <div className="pt-6 border-t border-white/5 flex flex-col gap-1">
              <span className="text-white/30 text-[10px] font-black tracking-widest uppercase">Verified Performance / {review.product}</span>
            </div>
          </div>
        ))}
        ...
        >
          ...
          <span className="text-accent font-black tracking-widest uppercase text-xs">View All</span>
        </Link>
      </div>
    </section>
  );
